...
import time, board, json, ssl, sys
		...
		if i2c_sensors:
			...
			for i in i2c_sensors:
				try:
					...
					if debug: 
						print('Error processing Multiplexer: '+str(e))
						sys.stdout.flush()
				...
				try:
					...
					if i2c_sensors[i]['type'] == 'BME680/688':
						...
						if i2c_sensors[i]['channel'] == 0:
							...
							if debug: 
								print('MS5607-02BA03 sensors can not be multiplexed')
								sys.stdout.flush()
			...
			if not instances:
				if debug: 
					print('Nothing to send, closing openplotter-i2c-read')
					sys.stdout.flush()
				return
						...
						for index, i in enumerate(instances):
							...
							if Erg:		
								...
								try: ws.send(SignalK+'\r\n')
									...
									if debug: 
										print('Error sending data to Signal K server:'+str(e))
										sys.stdout.flush()
									if ws: ws.close()
									ws = False
